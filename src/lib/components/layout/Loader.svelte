...
{#if visible}
	...
	>
		<div class="flex items-center gap-2 xl:gap-3">
			...
			<!-- Image plate (au lieu du SVG a pattern raster) : ce badge n'a jamais
			     ete un vrai vecteur, seulement un PNG deguise en SVG via un
			     <pattern>. Ce montage causait plusieurs bugs difficiles a
			     reproduire (fill noir sur Safari, deformations pendant
			     l'animation rotate/scale). Un <img> classique anime en CSS
			     n'a aucun de ces problemes. -->
			<div bind:this={pentagonEl} style="opacity: 0;" class="w-10 h-10 xl:w-14 xl:h-14">
				<img
					src="/assets/images/pentagon-badge.webp"
					alt=""
					aria-hidden="true"
					width="331"
					height="314"
